...
(defn on-change
  ...
  (when (not= "setValue" (.-origin change))
    #_(record-change! cm {:change (parse-change change)})     ;TODO VCR
...
(defn on-cursor-activity
  ...
  (when-not (frame-updated? cm)
    #_(record-change! cm {:selections (parse-selections (.listSelections cm))}) ;TODO VCR
    (fix-text! cm))
  ...
  ([element-id key- opts]
   (when-not (get @state key-)
     (let [element (js/document.getElementById element-id)
       ...
       #_(swap! vcr update-in [key-]                          ;TODO VCR
              #(or % empty-recording))
       ...
       (specify! cm
                 ...
                 #_(record-change! [this new-thing]           ;TODO VCR
